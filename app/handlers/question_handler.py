...
from data.supabase_client import get_user_progress, insert_question, supabaseClient, get_user_language
...
class QuestionHandler:
    ...
    async def give_next_question(self, wa_client, user_id: str):
        ...
        if domain.lower() == 'bot':
            language_code = await get_user_language(user_id) or 'id'
            print(language_code)
            try:
                prompt = (
                ...
                print(f"AI-generated question: {new_question}")
            ...
            print(f"Remaining questions in '{domain}': {len(remaining_questions)}")
